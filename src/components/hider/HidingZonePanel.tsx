  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-ink-muted">
        {moveMode
          ? "Pick a new transit station for your hiding zone."
      ...
      {stationsError ? <p className="text-error text-sm">{stationsError}</p> : null}
      ...
      {selectedStation ? (
        <p className="text-sm text-ink-secondary">
          Selected: <span className="font-medium">{selectedStation.name}</span>
        </p>
      ) : (
