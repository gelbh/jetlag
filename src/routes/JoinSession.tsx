...
import { isValidSessionCode, normalizeSessionCode } from '../services/sessionCodes'
  ...
  const handleJoin = async () => {
    ...
    try {
      if (isFirebaseConfigured()) {
        ...
        const session = await joinRemoteSessionByCode(normalized, user.uid)
        if (!session) {
  ...
  return (
    <main className="flex min-h-full flex-col justify-center gap-6 bg-slate-950 px-5 py-8">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-300">Enter the 4-letter code from the host.</p>
      </div>
