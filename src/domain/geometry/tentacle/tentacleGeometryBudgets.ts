...
export type TentacleBudgetResult =
  | { ok: true }
  | { ok: false; message: string };

export function assertTentaclePoiBudget(count: number): TentacleBudgetResult {
  if (count > TENTACLE_POI_MAX) {
    return { ok: false, message: TENTACLE_POI_OVER_BUDGET_MESSAGE };
  }
  return { ok: true };
}
