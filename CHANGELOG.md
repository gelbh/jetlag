...
- Pending questions on the hider map show recommended elimination shading while waiting for sync. Seeker and hider maps keep pending overlays visible during Firebase writes instead of a blank gap.
- Map: overlays and shading stay aligned while pinching and panning
- Map: transit and live-location icons no longer fail to register when the basemap reloads mid-load
- Map: restore game-area and question shading on MapLibre (layers bind to GeoJSON sources again)
- Map: fix touch/mouse pan stalling after a short nudge (camera stop no longer kills the active drag)
- Map: fix choppy pan and broken two-finger pinch zoom after MapLibre cutover
- Measuring: large multi-place questions (e.g. many parks) use a lighter disk union so phones are less likely to run out of memory
- Measuring: dense admin-border distance previews (for example Dublin LEAs) no longer crash the map on Further / step 3
- Measuring: over-dense place lists or border data are blocked with a clear message instead of freezing the map
- Session: if an answered question cannot be drawn, it cancels instead of crashing the map on every reload
- Measuring: overly complex shade regions are simplified when possible, or blocked with a clear message so the map does not freeze
- Map: keep location tools working when public Overpass is busy (extra mirrors + Postpass fallback).
- Session: queued annotation writes flush when you return to the app after backgrounding or a bfcache restore
- Map: restore smooth camera moves when opening question tools and updating placement drafts
- Matching: only places inside the play area count (no nearby expanded search)
- Radar: yes/no answers shade the eliminated side of the circle correctly (yes keeps the disk clear; no shades it)
- Game over rematch: switch roles keeps role codes working and shows a clear error if rematch is blocked
- Radar: answering Yes means within range (inside the circle for elimination).
- Map: overlays no longer crash the map when a layer’s source id changes mid-session
- Map chrome: raise the phone Session side stack so it no longer overlaps Zoom / Recenter
- Live map: seeker and hider pins fade as GPS goes quiet, show last-seen on the tooltip, and disappear after 60 minutes without an update
- Session exit: your live location pin is removed for other players when you leave or return home after game over
- Tentacle: answering a dense place list (e.g. many hospitals) no longer crashes the seeker map; failed draws cancel instead of retrying forever
- Tentacle: very large place lists are refused before heavy shading work so phones stay responsive

### Improvements

- Ask: question tools and hider zone use map HUD chrome (cue, chips/catalog, send strip) instead of floating wizard panels
- Map chrome: Session tools stack on the right above zoom; Hunt question tools stay visible without horizontal scroll.
...
- Session: role-code stamps with tap-to-reveal and copy from the Session island Codes sheet and Settings
- Join: request access when a side is gated; leaders accept or decline from the map alert
- Joining a side now requires a role code when that team already has players, or for observers. Hosts and team leaders can copy or regenerate codes from session settings.
- Board economy (ops admin): optional hider deck simulation - hand limit 6, sequential question rewards, power-ups, time bonuses, and curse clears. Off by default; only the ops admin account can enable it per session.
- Session settings: invite friends with a shareable join link. Opening the link prefills the session code on Join.
- Tools: question wizards use a shared Place → Configure → Ask flow with a phase rail and clearer commit actions
- Tools: thermometer Place requires enough pin-to-pin travel before Continue; Manual pins peek the sheet so map taps land
- Tools: wizard sheet peeks during Place and dims the map chrome so placement stays obvious
- Board hand: host can enable simulate-deck in Advanced rules (not ops-only). Hand sits next to Set zone; Move appears only with a Move card; answering shows a keep/discard draw pick.
- Found: seekers can still request Found while End Game is active.
- Found station: seekers start End Game immediately (no hider accept). In-zone questions answer from the hiding place; End Game banner stays alert-only.
- Hider map: keep Set zone / Expansion centered in the bottom dock; move Recenter onto the left stack with zoom and satellite
- Hiders can answer the open question from a map banner without opening chat. The chat badge for that question clears when they answer there.
- Hider Hunt: content-sized centered island so Set zone and Expansion labels stay readable
- Location: ask in-app why GPS is needed before the browser prompt, and show a clear blocked state with Try again instead of silent map-entry requests
- Map chrome: replace the dual-row tool dock with floating History, Hunt, Session, and Map-controls islands (Log + Recenter; Found/End only when eligible; More menu retired)
- Map: left-stack compass replaces Recenter; tap resets view and orientation; two-finger rotate
...
- Map: two-finger tilt is available for interactive maps when low-power mode is off (starts flat); low-power mode keeps the map flat
- Street map: measuring, tentacles, and matching place tools show map place previews right away, then confirm before you can send; tap near a labeled place to snap to it. Satellite keeps search only (no map place hints).
- Matching: recommended-preset place lists load from bundled data right away; live map data may still refresh quietly in the background.
- Presets: optional location pack attaches when your play area overlaps a supported city, with a quiet chip to clear or change and a request option when none match.
- Measuring: pack coastline and sea-level seeds load before Overpass/elevation enrich on region-pack presets
- Presets: more place types (zoos, libraries, and similar) load from recommended region packs when bundled data is available
- Pan hides left map controls and bottom/right chrome islands for seekers and hiders.
- Map: keep question tools open while panning during placement so thermometer A→B and other map taps still work
- Play Move: seekers see PHASE MOVE, the session timer pauses for everyone (including non-host hiders), and station search waits until you tap Search stations in this area
- Recommended presets: place lists for measuring and tentacles load from bundled data right away; live map data may still refresh quietly in the background.
- Custom presets: request a manual location preload review from the preset editor (not instant).
- Recommended presets: Expansion Pack starts off; hosts can still turn it on in session settings
- Question answers: clarify that recommended truth uses each hider's hiding-zone center until end game, then a frozen end-game GPS snapshot. Hider map shows an answer-reference pin at that point. End-game accept is blocked when any confirmed hider lacks a recent location.
- Game over rematch starts a fresh round: roles swap once, the map and live pins clear, and the hiding timer waits for the host
- Home: remove the tutorial page and Home guide entry
- Map: drop the first-run sheet link to the full tutorial
- Map: reopen map tools guide from Settings → Session
- Map: first-run map tools sheet dismisses with Got it
- App: redirect old /tutorial links to Home
- Role codes load faster when you open Codes and tap Reveal - the app warms the request in the background and the server fetches session data in parallel.
- Metric sessions: hiding zones 500 m (small/medium) or 1 km (large); radar maxima 10 / 40 / 160 km by size; thermometer 1/5, 1/5/15, or 1/5/15/75 km; game-size thresholds 250 / 2,500 km²
- Coastline measuring rule uses a 2 km waterway width; photo clearance and section sizes follow the session distance edition
- Measuring catalog defaults to the 20 book options; admin3/admin4 borders and custom place unlock with the custom question pack
- Question reuse: costs show as sequential draws (D3P1 ×2), not combined totals (D6P2).
- Sea level measuring: denser preset elevation seeds and local freehand refine so truthful closer/further answers keep the hider possible.
- Seeker dock: Undo and Redo bookend the Hunt tools, with equal-size question and Draw chips spanning the middle
- Seeker dock: Undo and Redo sit with equal-size question tools in the bottom-middle Hunt island; Draw moves to the right Session dock
- Map: show zoom for hiders and stack zoom above the satellite toggle on the left for everyone
...
- Admin ops desk: restore a Home control in the top bar to leave /admin
